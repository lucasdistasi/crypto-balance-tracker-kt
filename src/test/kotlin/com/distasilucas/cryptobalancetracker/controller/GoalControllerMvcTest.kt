...
class GoalControllerMvcTest(
  ...
  fun `should retrieve goals for page with status 200`() {
    ...
    mockMvc.retrieveGoalsForPage(0)
      ...
      .andExpect(jsonPath("$.goals[0].cryptoInfo.cryptoName", `is`("Bitcoin")))
      .andExpect(jsonPath("$.goals[0].cryptoInfo.cryptoId", `is`("bitcoin")))
      .andExpect(jsonPath("$.goals[0].cryptoInfo.symbol", `is`("btc")))
      .andExpect(jsonPath("$.goals[0].cryptoInfo.image", `is`("https://assets.coingecko.com/coins/images/1/large/bitcoin.png?1547033579")))
